> **LEGACY.** This is the Blender pipeline for the legacy hand-rigged `crab_hex.usd` model (used only by the Script Editor demos in `assets/scripts/`; the joystick/HAL task loads `assets/crab_hex_ref.usd`). The **training plant** is not produced this way: it is generated by `assets/scripts/generate_crab.py` (main asset `assets/crab.usda`, A15+B geometry). See [`docs/crab-hexapod-plant.md`](../docs/crab-hexapod-plant.md).
...
| **Blender-pipeline USD (checked in)** | `assets/crab_hex.usd` — the hand-rigged model this document describes (stage `/World/KrabbyUno`); loaded only by the Script Editor demos `assets/scripts/squat.py` / `simple_walk.py`, by no task or server |
| **Joystick/HAL model** | `assets/crab_hex_ref.usd` — produced from `assets/crab_hex_ref.urdf`; the default `KRABBY_HEX_USD_PATH` of `Isaac-CrabHex-Joystick-v0` (`extreme_parkour_task/config/hex/crab_hex_play_cfg.py`) and of `scripts/run_isaac_hal_server.sh` (`HEX_USD`); not the training plant |
