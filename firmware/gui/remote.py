...
def start_bridge(host: str, serial_dev: str = DEFAULT_SERIAL_DEV,
                 bridge_port: int = DEFAULT_BRIDGE_PORT,
                 remote_dir: str = DEFAULT_REMOTE_DIR) -> tuple[RemoteBridge, str]:
    """ssh-launch the serial/TCP bridge on a bench host and return
    (bridge, socket_url). Exits with the bridge's error message on failure;
    the caller owns bridge.stop()."""
    bridge = RemoteBridge(host, serial_dev=serial_dev,
                          bridge_port=bridge_port, remote_dir=remote_dir)
    try:
...
class RemoteBridge:
    ...
    def __init__(self, host: str, serial_dev: str = DEFAULT_SERIAL_DEV,
                 bridge_port: int = DEFAULT_BRIDGE_PORT,
