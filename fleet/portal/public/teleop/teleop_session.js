(function () {
  ...
  // Fleet portal sets TELEOP_CONFIG_PATH to /api/teleop/ice-servers; the existing default remains /api/teleop-config.
  var cfgPath = typeof window.TELEOP_CONFIG_PATH === 'string' ? window.TELEOP_CONFIG_PATH : '/api/teleop-config';
  var cfgUrl = httpProto + '//' + location.host + cfgPath + qs;
  ...
  function boot() {
    ...
    var cfgHeaders = {};
    if (token) {
      cfgHeaders['Authorization'] = 'Bearer ' + token;
    }
    fetch(cfgUrl, { headers: cfgHeaders })
      .then(function (r) {
  ...
  // Hooks for fleet Playwright E2E. Safe no-op for operators.
  window.__krabbyTeleop = {
    ...
    getControlDc: function () {
      return controlDc;
    },
    getWs: function () {
      return ws;
    },
    videoTrackCount: function () {
      return videosEl ? videosEl.querySelectorAll('video').length : 0;
    },
    /** Latest parsed frame off the krabby-telemetry-v1 channel (includes last_control echo). */
    getLastTelemetry: function () {
      return lastTelemetryMsg;
    },
    /**
     * Motion-safe control: RS (a leg-select button, zero stick axes -> zero
     * joint displacement either way) distinguishes this from the
     * RemoteGamepadState default so a poll of getLastTelemetry().last_control
     * can prove HAL actually received and applied it, not just that the
     * default state matched. operator_override stays false, so ControlLoop's
     * command_send_gate never forwards anything to hardware regardless.
     */
    sendMotionSafeControl: function () {
      if (!controlDc || controlDc.readyState !== 'open') {
        return false;
      }
      controlDc.send(
