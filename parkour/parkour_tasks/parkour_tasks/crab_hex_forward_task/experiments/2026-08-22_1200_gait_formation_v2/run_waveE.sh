#!/usr/bin/env bash
# PLAN E Phase 2 wave E (2026-08-22): first clock-reward screens on the collision-fixed plant.
set -u
...
declare -A ARMS=(
  [E1_clock]=""
  [E2_clock_clamp]="KRABBY_CAM_CLIP_LO=0.1"
  [E5_clock_nosym]="KRABBY_SYM_LOSS_COEF=0"
)
ORDER=(E1_clock E2_clock_clamp E5_clock_nosym)
...
for arm in "${ORDER[@]}"; do
  ...
  grep -oE "reward_clock_schedule: [0-9.]+" "$D/${arm}_train.log" | tail -1
done
echo "WAVE_E_DONE"
