...
"""Yaw-ROM probe: roll out a checkpoint and record the six Body_Hip yaw joints during walking.
...
parser.add_argument("--out", type=str, default=str(Path(__file__).parent / "yaw_rom_K1.npz"))
...
def main() -> None:
    ...
    yaw_ids, yaw_names = robot.find_joints([".*_Body_Hip_RevoluteJoint"], preserve_order=True)
    cam_ids, _ = robot.find_joints([".*_Body_CamShaft_RevoluteJoint"], preserve_order=True)
    print("[yawprobe] yaw joints:", yaw_names, flush=True)
    rows = {k: [] for k in ("yaw_pos", "cam_pos")}
    ...
    with torch.inference_mode():
        for step in range(args_cli.steps):
            ...
            if len(keep) == 0 or step % 3 != 0:  # thin the sampling
                continue
            rows["yaw_pos"].append(d.joint_pos[keep][:, yaw_ids].cpu().numpy())
            rows["cam_pos"].append(d.joint_pos[keep][:, cam_ids].cpu().numpy())
    bank = {k: np.concatenate(v).astype(np.float32) for k, v in rows.items()}
    bank["yaw_names"] = np.asarray(yaw_names)
    np.savez_compressed(args_cli.out, **bank)
    print(f"[yawprobe] {bank['yaw_pos'].shape[0]} walking states -> {args_cli.out}", flush=True)
