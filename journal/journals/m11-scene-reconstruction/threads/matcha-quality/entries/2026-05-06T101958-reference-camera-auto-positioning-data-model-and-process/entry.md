---
kind: entry
date: 2026-05-06T10:19:58-07:00
title: Reference-camera auto-positioning: data-model and process
mood: null
consolidates_notes:
  - journals/m11-scene-reconstruction/threads/matcha-quality/notes/2026-05-06T101958-auto-positioning-the-reference-camera-data-model-process-cha
tags: []
---
# Reference-camera auto-positioning: data-model and process

The 2026-05-04 planning pivot inserted Phase C *between* post-processing
and IsaacSim integration: validate that our pipeline reproduces MAtCha's
published reference-quality renders on the bicycle scene before treating
the meshes as ready for simulation. This entry consolidates the process
note for the 2026-05-06 implementation session — what changed in the
data model and pipeline shape to make Phase C work end-to-end. Two
sibling flat-file notes capture the same arc from other angles:
*planning-pivot-to-matcha-reference-validation* (the why, on 2026-05-04)
and *auto-localized-reference-cameras* (the results and findings, also
2026-05-06). This entry is the *how*.

## The starting friction

Phase C's first tangible artifact was supposed to be one Blender camera
at the same vantage as MAtCha's published bicycle hero shot, so we could
render our TSDF mesh from there and compare. While the agent was
unreachable for two days, the manual fallback ran: open
...
3. **No way to scale this.** Phase C wants reference cameras for
   eventually multiple scenes. Hand-placement is the throughput ceiling.
...
- Defaults preserve v3 semantics (any view without `purpose` is treated
  as `ab-comparison`).
...
The schema bump is silent (consumers handle missing fields), so the
existing 004-sky-house workflow keeps running without change.

## Round-trip mechanics: Blender custom properties

The harder problem: how does
...
The filter is applied at view enumeration in the shell wrapper, not
inside Python — keeps the change local.

## The auto-localization pipeline

What became `localize_reference_image.py` (404 lines):
...
2. **Sandbox is `<data-root>/sfm-ref-localize/<scene>/`, not under the
   scene's own dir.** Keeps the original
   `<scene>/mast3r_sfm/cameras.json` immutable; the SfM-with-ref output
   lives in a separate sibling. Anything else that consumes
   `mast3r_sfm/cameras.json` (other tooling, future training runs) sees
   an unchanged file.
...
We picked SfM-extend (variant 1 of three documented in the sibling
*Scaling M11 reconstruction beyond single-MAtCha runs* entry) because
the existing wrapper supports it directly. The other two variants stay
in design space:

- **PnP localization (variant 2).** The principled answer if shared-
  focal SfM fails on rendered references. Doesn't disturb the original
  12-camera reconstruction. The 0.74 m residual position delta in the
  bicycle test is the symptom that would push us here.
...
**Adding the auto-localizer was new code, not new infrastructure** — by
the time it landed, the data path it needed already existed.

## Open ends pointing back at the results note

The 0.74 m / 4.55° delta between manual and auto cameras is not a
process problem (the SfM math works — sub-cm Procrustes residuals); it's
a **shared-focal SfM measurement issue**, fully discussed in the sister
note *auto-localized-reference-cameras*. Whether that gap matters
enough to motivate Phase 2 (PnP, decoupled focal) or Phase 3
(differentiable refinement) is an open call that depends on whether
visual A/B/C comparison is good enough or pixel-aligned comparison is
required.
