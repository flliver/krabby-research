...
| `003-firepit/slam3r` | **measured** | 2026-04-12T06:18:18-07:00 | slam3r_output artifact present ONLY on dbeeprz (outposts partial per-host tree); on-disk mtime | MEASURED host/date: ran on dbeeprz (RTX 4080) — slam3r_output lives only there; date 04-12. Params still unrecoverable (no run-script). |
...
**Summary:** 8 measured, 4 deduced (of 12 legacy transforms). Records enriched with real dates + script-derived params even where provenance stays `deduced`.
